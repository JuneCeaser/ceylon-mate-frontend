...
import {
  ...
  Dimensions, Alert, FlatList, ScrollView, RefreshControl, Linking, Platform
} from 'react-native';
...
// ── Deep link scheme must match AndroidManifest.xml ──
const AR_APP_DEEP_LINK = 'ceylonAR://launch';
...
export default function PlaceScreen() {
  ...
  const fetchNearbyPlace = async (lat, lng) => {
    try {
      const response = await axios.get(`${API_URL}/nearby`, { params: { lat, lng } });
  ...
  // ── Open the CeylonAR Android app via deep link ──
  const openARApp = async () => {
    if (Platform.OS !== 'android') {
      Alert.alert('Not Supported', 'AR View is only available on Android devices.');
      return;
    }
    try {
      const supported = await Linking.canOpenURL(AR_APP_DEEP_LINK);
      if (supported) {
        await Linking.openURL(AR_APP_DEEP_LINK);
      } else {
        Alert.alert(
          'CeylonAR Not Found',
          'Please make sure the CeylonAR app is installed on your device.',
          [{ text: 'OK' }]
        );
      }
    } catch (error) {
      console.error("AR Launch Error:", error);
      Alert.alert('Error', 'Could not open the AR app. Please try again.');
    }
  };
        ...
        <View style={styles.imageCard}>
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <TouchableOpacity activeOpacity={0.9} onPress={() => { setCurrentImageIndex(index); setVisible(true); }}>
                <Image source={{ uri: item }} style={{ width: width - 40, height: 250 }} resizeMode="cover" />
              </TouchableOpacity>
            )}
          />
        ...
        >
          <View style={styles.scannerBannerLeft}>
            ...
            <View>
              ...
              <Text style={styles.scannerBannerSub}>Point camera at any stone artifact</Text>
            </View>
          </View>
        ...
        <View style={styles.gridContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              if (nowLink || thenLink) {
                router.push({ pathname: '/3d-model', params: { nowUrl: nowLink, thenUrl: thenLink } });
              } else {
          ...
          <TouchableOpacity
            style={styles.card}
            onPress={() => router.push({ pathname: '/place-chat', params: { placeId: place._id, placeName: place.name } })}
          >
            <View style={styles.iconCircle}>
          ...
          <TouchableOpacity
            style={styles.card}
            onPress={() => router.push({ pathname: '/facts', params: { placeData: JSON.stringify(place) } })}
          >
            <View style={styles.iconCircle}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (place.location && place.location.coordinates) {
                router.push({
                  pathname: '/map',
                  params: { lat: place.location.coordinates[1], lng: place.location.coordinates[0], name: place.name }
                });
              } else {
...
const styles = StyleSheet.create({
  ...
  scannerIconCircle: {
    width: 46, height: 46, backgroundColor: 'rgba(0,0,0,0.1)',
  ...
  scannerAiTag: {
    backgroundColor: '#000', color: '#FACC15',
  ...
  scannerBtnAlone: {
    marginTop: 16, backgroundColor: '#FACC15',
